...
function drupal_cms_installer_choose_template(array &$install_state): ?array {
  ...
  $install_state['recipes'] = iterator_to_array($recipes);

  // Put the blank site template first.
  // @todo Remove this if and when `drupal_cms_site_template_base` is no longer shipped
  //   with Drupal CMS.
  $first = 'drupal_cms_site_template_base';
  uksort(
    $install_state['recipes'],
    fn (string $a, string $b): int => $a === $first ? -1 : ($b === $first ? 1 : 0),
  );
